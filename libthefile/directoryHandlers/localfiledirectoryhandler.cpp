 ...
 *
 * *************************************/
#include "localfiledirectoryhandler.h"
...
DirectoryPtr LocalFileDirectoryHandler::parentDirectoryForUrl(QUrl url) {
    if (url.isLocalFile()) {
...
QString LocalFileDirectoryHandler::relativePath(QUrl from, QUrl to) {
    if (from.scheme() != "file" && to.scheme() != "file") return "";
    return QDir(from.toLocalFile()).relativeFilePath(to.toLocalFile());
}
